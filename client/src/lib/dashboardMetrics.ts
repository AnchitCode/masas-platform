 ...
 * }}
 */
export function computeOperationalMetrics(inventory: InventoryItem[]) {
 ...
 */
export function buildOperationalTimeline(inventory: InventoryItem[], pharmacy?: Pharmacy) {
  const rows = Array.isArray(inventory) ? inventory : [];
  const events: Array<{ id: string; tone: string; title: string; detail: string }> = [];
 ...
 */
export function buildAiInsights(metrics: ReturnType<typeof computeOperationalMetrics>) {
  const insights: Array<{ id: string; title: string; confidence: number; body: string; action: string; href: string }> = [];
