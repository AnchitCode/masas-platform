...
const preprocessFloat = (val: unknown): unknown => {
...
const searchInventoryQuerySchema = z.object({
  ...
  lat: z.preprocess(
    preprocessFloat,
    z.number({ error: 'Latitude must be a number' }).min(-90).max(90)
  ),
  lng: z.preprocess(
    preprocessFloat,
    z.number({ error: 'Longitude must be a number' }).min(-180).max(180)
  ),
  ...
  page: z.preprocess(
    ...
    z.number({ error: 'Page must be a number' }).int().min(1).optional().default(1)
  ),
  limit: z.preprocess(
    (val) => (val === undefined || val === '' ? undefined : preprocessInt(val)),
    z
...
export type SearchInventoryQuery = z.infer<typeof searchInventoryQuerySchema>;
